...
var (
	commands = []struct {
		...
		{Query: "REFRESH MATERIALIZED VIEW CONCURRENTLY statistic_txn_count_30_d", Ticker: time.NewTicker(30 * time.Minute)},
		...
		{Query: "REFRESH MATERIALIZED VIEW CONCURRENTLY statistic_active_wallet_count_30_d", Ticker: time.NewTicker(2 * time.Hour)},
		{Query: "REFRESH MATERIALIZED VIEW CONCURRENTLY statistic_block_height_current", Ticker: time.NewTicker(2 * time.Second)},
		...
		{Query: "SELECT refresh_public_key_first_transaction()", Ticker: time.NewTicker(1 * time.Hour)},
