...
func init() {
	initial_migrations.Migrations.MustRegister(func(ctx context.Context, db *bun.DB) error {
		_, err := db.Exec(`
			...
			comment on table balance_entry is E'@foreignKey (hodler_pkid) references account (pkid)\n@foreignKey (creator_pkid) references account (pkid)';
			...
			comment on table follow_entry is E'@foreignKey (follower_pkid) references account (pkid)\n@foreignKey (followed_pkid) references account (pkid)';
