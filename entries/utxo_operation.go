	...
	for ii, entry := range uniqueEntries {
		...
		for jj, utxoOps := range utxoOperations.UtxoOpBundle {
			...
			if jj < len(transactions) {
				...
				//fmt.Printf("HERE IS THE TX INDEX METADATA: %+v\n", txIndexMetadata)

				//if txIndexMetadata.SubmitPostTxindexMetadata != nil {
				//	fmt.Printf("HERE IS THE TX INDEX SUBMIT POST METADATA: %+v\n", txIndexMetadata.SubmitPostTxindexMetadata)
				//}
